...
- **Source order:** `source "$DIR/helpers.sh"` first. That brings in `get_tmux_option`, `useful_int_option`, `useful_icon_option`, `cache_check`, `useful_cache_dir`, `useful_timeout`, `useful_escape`, `useful_hash`, `is_darwin`/`is_linux`, `segment_enabled`, and
  `color_*`.
...
- **Key a cache with `useful_hash`, never a bare `| shasum |`.** `shasum` is a perl script; musl distros ship `sha1sum` instead. When it is absent the inline form yields an EMPTY key, and an empty key is not a degraded key — it is a colliding one. Every repo shared the single `git-` entry and the segment showed the wrong branch for the whole TTL.
- **Bound an external response at INGEST, not at render.** `USEFUL_MAX_CELLS` caps what reaches the bar; it does not cap what reaches the cache. A body stored verbatim is re-read and re-sliced on every refresh for the whole TTL — an 8MB wttr.in splash cost 1.07s of CPU per status tick before `--max-filesize` and `head -c` were added.
...
- **Characters are not cells.** `${#s}` and `${s:0:n}` count characters; the status bar's budget is display columns. A CJK glyph is 2 cells, a combining mark 0, and `U+FE0F` widens the glyph before it. Plenty of wide glyphs carry no `U+FE0F` at all — `⌚ ⚡ ⛅ 🈚` are two cells bare, and wttr.in's `%c` emits them by default — so the width table owes them explicit ranges; leaning on the variation-selector promotion undercounted, which is the direction that OVERFLOWS the budget. Never truncate by hand — use `useful_truncate` / `useful_window` from `helpers.sh`.
...
- **`read -r a b` gives the LAST variable the whole remainder, delimiters included.** The Spotify state file was written track-first, so a title containing `|` — common — split across both fields, the stored title never matched, and the track-change branch fired on every refresh. Put the field that cannot contain the delimiter first, and validate it on the way back in.
- **A directory you created is not a directory you own.** `mkdir -p` succeeds on an existing directory whoever owns it, and the cache path is derived from a predictable name. A cache entry is markup pasted into the status line, so an unowned cache dir is a write primitive on the status line: `useful_cache_dir` creates with mode 700 and refuses anything that fails `-O`.
- **A bats assertion that is not the last line of a test silently passes on bash 3.2**, which is what macOS ships and what you are probably running locally. CI installs bash 5 precisely so the assertion aborts; that gap hid a broken test through a whole release. When a test drives one segment across several bands, `rm -f "$TMUX_USEFUL_CACHE_DIR/<segment>"` between them — the segment caches for seconds, so without it the first band's result is replayed into every later assertion and only the trailing one is ever checked.
